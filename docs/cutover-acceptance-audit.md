...
- `uv run linkedin-tools network acceptance --help`
- `uv run linkedin-tools network pending-cleanup --help`
...
| 2 | `linkedin-tools` exposes all planned namespaces plus cutover readiness checks. | Proven | `uv run linkedin-tools --help` listed `network`, `recruiter-agency`, `opportunity`, `comments`, `ui`, and `cutover`; each namespace help command succeeded; cutover audit and prompt edit-plan commands succeeded. |
