...
export interface NetworkScriptInput {
  ...
  /** Pending attempt names that must be present for confirmation; the sent-list
   * capture stops scrolling once all are found instead of loading the full
   * historical sent page. Only valid for capture-sent-list. */
...
function walkListBody(
  ...
  return [
    ...
    `return {firstPage,replaySearch,firstTotal:Number(firstPage.paging?.total)||0,hdrs};`,
    `};`,
...
function sentListCaptureBody(sentUrl: string, confirmNames: readonly string[] | undefined): string {
  const confirmNamesJson = literal(confirmNames ?? []);
  const body = [
...
export function compileNetworkScript(
  ...
  if (input.confirmNames !== undefined && command !== "capture-sent-list") {
    throw new TypeError("confirmNames is only valid for capture-sent-list");
  }
  if (
  ...
  switch (command) {
    ...
    case "capture-sent-list":
      return networkIssue(
        ...
        sentListCaptureBody(sentUrl, input.confirmNames),
      );
    case "walk-list": {
      const u = needSearch();
      const sourceContract = needSourceContract();
