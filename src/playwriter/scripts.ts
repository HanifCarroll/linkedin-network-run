...
function walkListBody(
  ...
  return [
    ...
    `const maxPages=10;`,
    `const p=state[${literal(WORKFLOW_STATE_KEYS.candidateResults)}]??page;`,
    `state[${literal(WORKFLOW_STATE_KEYS.candidateResults)}]=p;`,
  ...
  switch (command) {
    ...
    case "navigate-sent-list":
      return networkIssue(
        ...
        `${before}${progress("navigation_started")}const p=state[${literal(WORKFLOW_STATE_KEYS.sentInvitations)}]??page;state[${literal(WORKFLOW_STATE_KEYS.sentInvitations)}]=p;await p.goto(${literal(sentUrl)},{waitUntil:"domcontentloaded"});${progress("navigation_returned")}if(p.url()!==${literal(sentUrl)})throw new Error("WRONG_PAGE");${after}${finish(command, "data:{url:p.url()}")}`,
      );
