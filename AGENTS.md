...
- `jobs followup-next` is a read-only post-Day-1 HubSpot task packet. A completed Instantly email/lead produces only a monitoring task; no-email produces LinkedIn checks for days 5–7, 8–10, and 12–14 with reply/connection gates. `jobs followup-record` stores explicit company/contact/deal-associated task IDs. The CLI never calls HubSpot, mutates CRM, schedules locally, or duplicates Instantly cadence; any reply stops the sequence.
