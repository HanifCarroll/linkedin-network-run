...
- LinkedIn Jobs capture, direct-page enrichment, and outreach use caller-owned Codex Chrome handoffs plus CLI SQLite ingest. Direct enrichment is request-first: arm CDP before navigation, retain only the target Document and aboutTheJob/aboutTheCompanyForJobDetails/peopleWhoCanHelp response bodies, parse locally, then verify with stable DOM reads. Jobs outreach uses jobs send-prepare/send-record; contract outreach additionally uses jobs contract-outreach-prepare/record for approved direct prospects or approved applied application-followup prospects. Arm before visible UI, retain only bounded request metadata and thread/invitation confirmation, never cookies/headers/private write-XHR replay. Never add CLI browser control, browser leases, or cross-automation locks. Jobs liveness checking, networking, and analytics remain on Playwriter.
...
- Never replace a possible send or infer failure from absence. Jobs send-prepare and contract-outreach-prepare reserve one prepared attempt before the caller acts; a handoff without exact bounded request evidence and thread/invitation-visible confirmation remains possible and blocks retry until reconciled. Contract-outreach endpoint contracts remain empty/fail-closed until learned live. `proven_no_send` is the only safe release; there is no failed terminal state.
