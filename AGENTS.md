...
- LinkedIn Jobs capture, direct-page enrichment, and outreach use caller-owned Codex Chrome handoffs plus CLI SQLite ingest. Direct enrichment is request-first: arm CDP before navigation, retain only the target Document and aboutTheJob/aboutTheCompanyForJobDetails/peopleWhoCanHelp response bodies, parse locally, then verify with stable DOM reads. Jobs outreach uses jobs send-prepare/send-record; contract outreach additionally uses jobs contract-outreach-prepare/record for approved applied contract prospects. Arm before visible UI, retain only bounded request metadata and thread/invitation confirmation, never cookies/headers/private write-XHR replay. Never add CLI browser control, browser leases, or cross-automation locks. Jobs liveness checking, networking, and analytics remain on Playwriter.
