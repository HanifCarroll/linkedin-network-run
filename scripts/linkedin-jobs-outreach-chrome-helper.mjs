...
export async function sendContractOutreachInvitation(tab, config) {
  ...
  for (let i = 0; i < 30; i += 1) {
    const batch = await cdp.readEvents({
      ...
      limit: 200,
      timeoutMs: 1000,
      methods: ["Network.requestWillBeSent", "Network.responseReceived", "Network.loadingFailed"],
    });
    cursor = batch.cursor;
    if (batch.truncated) return { ok: false, reason: "cdp-truncated" };
    for (const event of batch.events) {
      ...
      if (
        ...
        String(p.request.url) === config.endpointUrl
      ) {
        const body = typeof p.request.postData === "string" ? p.request.postData : "";
        request = {
          requestId: String(p.requestId),
          method: "POST",
          url: String(p.request.url),
          bodySha256: sha(body),
          recipientUrn: body.match(/urn:li:[^"'&\\]+/)?.[0] ?? "",
        };
      }
      if (
        event.method === "Network.responseReceived" &&
        request &&
        String(p.requestId) === request.requestId
      )
        response = { status: Number(p.response?.status ?? 0) };
    }
    if (request && response) break;
  }
  const invitation = await config.invitationCheck();
  const confirmed = Boolean(
    request &&
      response &&
      response.status >= 200 &&
...
export async function sendAndRecordContractOutreachInvitation(tab, config) {
  const record = (state, evidence) =>
    pipeRawBodyToCli(
      JSON.stringify({
        attemptId: config.packet.attemptId,
        jobId: config.packet.jobId,
        route: config.packet.route,
        draftFingerprint: config.packet.draftFingerprint,
        state,
        evidence,
      }),
      [
  ...
  const result = await sendContractOutreachInvitation(tab, {
    ...config,
    beforeAction: () => record("possible", { commitStarted: true }),
  });
  if (!result.ok || result.state === "possible") return result;
  return record("confirmed", result.evidence);
}
